...
- ✅ 프로젝트별 주간 업무 요약 (Gemini API)
- ✅ 프로젝트별 우선순위 추천 (Gemini API)
- ✅ 프로젝트 목록 화면 자연어 Task 검색 (structured intent + semantic recall)
...
GEMINI_SEARCH_EMBEDDING_MODEL=gemini-embedding-001
GEMINI_SEARCH_SEMANTIC_ENABLED=true
GEMINI_SEARCH_EMBEDDING_DIMENSIONS=3072
...
자연어 검색의 semantic recall을 사용하려면 PostgreSQL에 `pgvector` extension이 활성화되어 있어야 합니다.
현재 로컬 Docker 환경에서는 이를 기준으로 검색 embedding을 저장합니다.

기본값으로는 주간 요약 캐시가 비활성화되어 있습니다.
Redis 캐시를 사용하려면 `WEEKLY_SUMMARY_CACHE_ENABLED=true` 와 `REDIS_URL`을 함께 설정해야 합니다.
...
export GEMINI_SEARCH_EMBEDDING_MODEL=gemini-embedding-001
```
...
- [x] **Phase 2**: 프로젝트별 주간 업무 요약 (Gemini)
- [x] **Phase 3**: 프로젝트별 우선순위 추천 (Gemini)
- [x] **Phase 4 v1**: 자연어 Task 검색 (`/projects`, Gemini + pgvector)
...
- `GET /api/projects/weekly-summary/cache-health` - Redis read/write 헬스 체크

### LLM Recommendation
- `GET /api/projects/{projectId}/task-recommendations` - 프로젝트 우선순위 추천 생성

### Natural Language Search
- `POST /api/search/tasks` - 프로젝트 목록 화면 자연어 Task 검색

## 주간 요약 동작 방식

- 주간 요약은 `동기화된 일정` / `미동기화 일정` 2개 섹션으로 나뉘어 생성됩니다.
- 분류는 단순 `calendarEventId` 존재 여부가 아니라 최신 Outbox 상태까지 반영해 판정합니다.
- 요약 입력은 모든 Task를 그대로 보내지 않고, 우선순위가 높은 대표 Task subset만 선별해서 보냅니다.
- 프롬프트는 `includedTaskCount`와 `totalTaskCount`를 함께 보내서, 대표 subset 기반 요약이라는 점을 모델에 명시합니다.
...
- 캐시 키는 `exact`와 `latest` 두 종류를 사용합니다.
...
- semantic recall은 `pgvector`와 Gemini embedding을 사용하지만, 주 엔진이 아니라 보조 recall 층으로만 사용합니다.
- 참여자 조건은 specific participant와 generic companion을 분리합니다.
